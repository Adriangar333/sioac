...
export class DatabaseStorage implements IStorage {
  ...
  async createClient(client: InsertClient): Promise<Client> {
    return db.insert(clients).values({ ...client, createdAt: now(), updatedAt: now() }).returning().get();
  }
  async updateClient(id: number, data: Partial<InsertClient>): Promise<Client | undefined> {
    return db.update(clients).set({ ...data, updatedAt: now() }).where(eq(clients.id, id)).returning().get();
  }
  ...
  async getClientImages(clientId: number): Promise<ClientImage[]> {
    return db.select().from(clientImages).where(eq(clientImages.clientId, clientId)).all();
  }
  async createClientImage(image: InsertClientImage): Promise<ClientImage> {
    return db.insert(clientImages).values({ ...image, createdAt: now() }).returning().get();
  }

  // Searches
  async getSearches(): Promise<Search[]> {
    return db.select().from(searches).orderBy(desc(searches.createdAt)).all();
  }
  ...
  async getSearchesByClient(clientId: number): Promise<Search[]> {
    return db.select().from(searches).where(eq(searches.clientId, clientId)).orderBy(desc(searches.createdAt)).all();
  }
  async createSearch(search: InsertSearch): Promise<Search> {
    return db.insert(searches).values({ ...search, createdAt: now() }).returning().get();
  }
  ...
  async getSearchQueries(searchId: number): Promise<SearchQuery[]> {
    return db.select().from(searchQueries).where(eq(searchQueries.searchId, searchId)).all();
  }
  async createSearchQuery(query: InsertSearchQuery): Promise<SearchQuery> {
    return db.insert(searchQueries).values({ ...query, createdAt: now() }).returning().get();
  }

  // Results
  async getResults(searchId: number): Promise<Result[]> {
    return db.select().from(results).where(eq(results.searchId, searchId)).orderBy(desc(results.createdAt)).all();
  }
  ...
  async getResultsByClient(clientId: number): Promise<Result[]> {
    return db.select().from(results).where(eq(results.clientId, clientId)).all();
  }
  async createResult(result: InsertResult): Promise<Result> {
    return db.insert(results).values({ ...result, createdAt: now(), updatedAt: now() }).returning().get();
  }
  async updateResult(id: number, data: Partial<InsertResult>): Promise<Result | undefined> {
    return db.update(results).set({ ...data, updatedAt: now() }).where(eq(results.id, id)).returning().get();
  }
  async getAllResults(): Promise<Result[]> {
    return db.select().from(results).orderBy(desc(results.createdAt)).all();
  }

  // Classification Rules
  async getRules(): Promise<ClassificationRule[]> {
    return db.select().from(classificationRules).orderBy(classificationRules.priority).all();
  }
  async getRule(id: number): Promise<ClassificationRule | undefined> {
    return db.select().from(classificationRules).where(eq(classificationRules.id, id)).get();
  }
  async createRule(rule: InsertClassificationRule): Promise<ClassificationRule> {
    return db.insert(classificationRules).values({ ...rule, createdAt: now() }).returning().get();
  }
  async updateRule(id: number, data: Partial<InsertClassificationRule>): Promise<ClassificationRule | undefined> {
    return db.update(classificationRules).set(data).where(eq(classificationRules.id, id)).returning().get();
  }
  async deleteRule(id: number): Promise<void> {
    db.delete(classificationRules).where(eq(classificationRules.id, id)).run();
  }

  // Export Jobs
  async getExportJobs(): Promise<ExportJob[]> {
    return db.select().from(exportJobs).orderBy(desc(exportJobs.createdAt)).all();
  }
  ...
  async createExportJob(job: InsertExportJob): Promise<ExportJob> {
    return db.insert(exportJobs).values({ ...job, createdAt: now() }).returning().get();
  }
  async updateExportJob(id: number, data: Partial<InsertExportJob>): Promise<ExportJob | undefined> {
    return db.update(exportJobs).set(data).where(eq(exportJobs.id, id)).returning().get();
  }
  ...
  async getAuditLogs(entityType?: string, entityId?: number): Promise<AuditLog[]> {
    if (entityType && entityId) {
      return db.select().from(auditLog)
        ...
        .orderBy(desc(auditLog.createdAt)).all();
    }
    return db.select().from(auditLog).orderBy(desc(auditLog.createdAt)).limit(100).all();
  }
  async createAuditLog(log: InsertAuditLog): Promise<AuditLog> {
    return db.insert(auditLog).values({ ...log, createdAt: now() }).returning().get();
  }

  // Dashboard Stats
  async getDashboardStats(): Promise<any> {
    const totalResults = db.select({ count: count() }).from(results).get();
    const allResults = db.select().from(results).all();
    const totalClients = db.select({ count: count() }).from(clients).get();
    const totalSearches = db.select({ count: count() }).from(searches).get();
...
export function seedDemoData() {
  ...
  const client1 = db.insert(clients).values({
  ...
  const client2 = db.insert(clients).values({
  ...
  const client3 = db.insert(clients).values({
  ...
  // Create 2 searches
  const search1 = db.insert(searches).values({
    clientId: client1.id,
    createdBy: 1,
    status: "completed",
    totalQueries: 24,
  ...
  const search2 = db.insert(searches).values({
    clientId: client2.id,
    createdBy: 1,
    status: "completed",
    totalQueries: 18,
  ...
  db.insert(classificationRules).values({
